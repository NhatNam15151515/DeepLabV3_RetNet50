...
# Đảm bảo project root (chứa config.py, model_setup.py, ...) nằm trên sys.path
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

"""
api.py — FastAPI Prediction Server cho Food Segmentation Demo.

Chạy (sau khi gom script vào scripts/api):
...
FOOD_DENSITIES = {
    "egg": 1.03, "banana": 0.95, "steak": 1.08, "pork": 1.05,
    "chicken": 1.05, "fish": 1.02, "shrimp": 0.85, "bread": 0.35,
...
# Thư mục lưu kết quả API (ảnh + json) cho mỗi request
SCRIPTS_DIR = os.path.join(PROJECT_ROOT, "scripts")
API_SAVE_DIR = os.path.join(SCRIPTS_DIR, "api_results")
os.makedirs(API_SAVE_DIR, exist_ok=True)

# Kích thước tối đa cạnh dài (model train 512, giữ để tiết kiệm VRAM)
MAX_INPUT_SIDE = 512
...
# =====================================================================
# COLOR PALETTE — 1 màu riêng biệt cho mỗi class (theo FoodSemSeg_512x512)
# =====================================================================
CLASS_COLORS = [
    (0,   0,   0),      # 0  background
    (255, 255, 255),    # 1  egg
...
def preprocess_any_size(image: np.ndarray, max_side: int = MAX_INPUT_SIDE):
    ...
    # Pad về max_side x max_side (pad bottom + right)
...
class ModelSingleton:
    ...
    def __init__(self):
        ...
        print(f"[API] Seg model ready on {self.device} | classes={CFG.NUM_CLASSES}")

        print("[API] Loading Tier2 (MiDaS) + Tier3 (Weight)...")
...
app = FastAPI(title="FoodSegmentation API", lifespan=lifespan)
...
async def predict(file: UploadFile = File(...)):
    ...
    # Đếm instance per-class (dùng preds_orig để đếm đúng)
    inst_dict = count_instances(preds_orig, CFG.NUM_CLASSES)
    class_names = getattr(CFG, "CLASS_NAMES", [f"class_{i}" for i in range(CFG.NUM_CLASSES)])

    # Tier 2-3: ước lượng trọng lượng cho từng class có mặt
    weight_map = {}  # {class_id: weight_grams}
    ...
    # Vẽ tên + trọng lượng trực tiếp lên vùng mask
    if present_classes:
        color_mask = draw_labels_on_regions(color_mask, preds_orig, present_classes, class_names, weight_map)
        overlay = draw_labels_on_regions(overlay, preds_orig, present_classes, class_names, weight_map)
    ...
    try:
        ...
        out_base = os.path.join(API_SAVE_DIR, safe_stem)
        ...
        print("[API] Warning: failed to save api_results:", repr(e))
